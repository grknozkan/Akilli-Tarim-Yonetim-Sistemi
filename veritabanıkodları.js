...
require('pg-hstore'); 
const { Op } = require('sequelize'); // YENİ EKLENDİ: Gelişmiş veritabanı sorguları için
...
const runSimulationUpdate = async () => {
  try {
    ...
    if (!lastSensor || lastSensor.lastUpdate < fiveMinutesAgo) {
      ...
      // YENİ EKLENDİ: Simülasyon çalıştığında Log tablosuna kayıt at
      await Log.create({
        action: 'SİSTEM GÜNCELLEMESİ',
        details: `${sensors.length} adet sensör verisi otomatik olarak güncellendi.`,
        timestamp: new Date()
      }).catch(err => console.error('Log yazma hatası:', err)); // Tablo yoksa çökmeyi önler
    }
  } catch (err) {
...
router.get('/dashboard/summary', async (req, res) => {
  await runSimulationUpdate();
  try {
    const sensors = await Sensor.findAll();
    res.json({
      success: true,
      data: {
        totalFields: sensors.length, // Statik 4 yerine dinamik sayı eklendi
        activeAlerts: sensors.filter(s => s.moisture < 40 || s.temp > 30).length, // Uyarı mantığı geliştirildi
        overallHealth: '95%',
...
// YENİ EKLENDİ: Acil Müdahale Gerektiren Tarlaları Listeleme (Alerts)
router.get('/sensors/alerts', async (req, res) => {
  await runSimulationUpdate();
  try {
    const alerts = await Sensor.findAll({
      where: {
        [Op.or]: [
          { moisture: { [Op.lt]: 45 } }, // Nem %45'ten düşükse
          { temp: { [Op.gt]: 30 } },     // Sıcaklık 30 dereceden büyükse
          { ph: { [Op.lt]: 5.5 } }       // pH 5.5'ten düşükse
        ]
      }
    });
    res.json({ success: true, count: alerts.length, data: alerts });
  } catch (err) {
    console.error('API Error (/sensors/alerts):', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// YENİ EKLENDİ: Dışarıdan Sisteme Yeni Sensör (Tarla) Ekleme
router.post('/sensors', async (req, res) => {
  try {
    const { label, x, y, temp, moisture, ph } = req.body;
    if (!label) {
      return res.status(400).json({ success: false, message: "Sensör adı (label) zorunludur." });
    }
    const newSensor = await Sensor.create({ label, x, y, temp, moisture, ph });
    res.status(201).json({ success: true, data: newSensor, message: "Yeni sensör başarıyla eklendi." });
  } catch (err) {
